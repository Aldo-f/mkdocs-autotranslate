...
def test_parse_post_no_frontmatter(docs):
    p = docs / "en"
    p.mkdir(parents=True)
    (p / "x.md").write_text("just prose", encoding="utf-8")
    assert parse_post(p / "x.md") == (None, None)
...
def test_find_gaps_both_directions_and_drafts(docs):
    ...
    assert ("en", "nl", "only-en") in triples
    assert ("nl", "en", "only-nl") in triples
    assert ("en", "nl", "draft-post") not in triples
    assert drafts == [("en", "nl", "draft-post")]
...
def test_fill_gaps_write_creates_translated_once(docs):
    ...
    assert "translated from `en/p1`" in text  # provenance
